...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function handleApplicationSubmit(e) {
    e.preventDefault();
    if (!currentUser) {
      displayMessage("Debe iniciar sesión para aplicar a este proyecto", "error");
      return;
    }
    ...
    if (!message || !proposedBudget || !proposedTimeframe) {
      displayMessage("Por favor complete todos los campos del formulario", "warning");
      return;
    }
    ...
    try {
      ...
      displayMessage("Su propuesta ha sido enviada exitosamente", "success");
      applyBtn.disabled = false;
      applyBtn.innerHTML = "Enviar Propuesta";
    } catch (error) {
      console.error("Error al enviar propuesta:", error);
      displayMessage("Error al enviar su propuesta. Por favor intente nuevamente.", "error");
      applyBtn.disabled = false;
      applyBtn.innerHTML = "Enviar Propuesta";
    }
  }
  function confirmDeleteProject() {
    ...
    if (confirmed) {
      displayMessage("Funcionalidad de eliminación pendiente de implementar", "info");
    }
  }
  function saveProject() {
    if (!currentUser) {
      displayMessage("Debe iniciar sesión para guardar este proyecto", "error");
      return;
    }
      ...
      .then((docSnap) => {
        if (docSnap.exists()) {
          return deleteDoc(savedProjectRef).then(() => {
            ...
            displayMessage("Proyecto eliminado de guardados", "success");
          });
        } else {
          return setDoc(savedProjectRef, {
            ...
            saveBtn.innerHTML = '<i class="fas fa-bookmark"></i> Guardado';
            saveBtn.disabled = false;
            displayMessage("Proyecto guardado exitosamente", "success");
          });
        }
      })
      .catch((error) => {
        ...
        saveBtn.disabled = false;
        displayMessage("Error al procesar su solicitud. Por favor intente nuevamente.", "error");
      });
  }
  function shareProject() {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard
        ...
        .then(() => {
          displayMessage("Enlace copiado al portapapeles", "info");
        })
        .catch((err) => {
  ...
  function startChat() {
    if (!currentUser) {
      displayMessage("Debe iniciar sesión para contactar al Cliente", "warning");
      closeContactModal();
      return;
    }
    if (contractorData && contractorData.id) {
      ...
      displayMessage("No se pudo iniciar el chat", "error");
      closeContactModal();
    }
  }
