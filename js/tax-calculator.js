...
function saveAsDraft() {
  if (!currentUser) {
    displayMessage("Debe iniciar sesión para guardar cálculos.","warning");
    return;
  }
...
async function saveCalculation(event) {
  ...
  if (form && !form.checkValidity()) {
    displayMessage("Por favor complete todos los campos requeridos antes de guardar.","warning");
    return;
  }
  const calculationData = collectFormData();
...
async function saveCalculationToFirebase(calculationData) {
  try {
    ...
    displayMessage("Error al guardar el cálculo. Por favor intente nuevamente.","error");
    return null;
  }
}
...
async function sendCalculationToClient() {
  ...
  displayMessage(`Enviando cálculo al cliente ${clientEmail}...`,"info");
  setTimeout(() => {
    try {
      ...
      displayMessage("Cálculo enviado correctamente al cliente.","success");
    } catch (error) {
      console.error("Error al marcar el cálculo como enviado:", error);
      displayMessage("El cálculo ha sido enviado, pero hubo un error al actualizarlo en la base de datos.","error");
    }
  }, 2000);
}
function exportCalculations() {
  if (calculationHistory.length === 0) {
    displayMessage("No hay cálculos para exportar.","error");
    return;
  }
...
function importFromExcel() {
  displayMessage("La importación desde Excel está en desarrollo. Esta funcionalidad permitirá cargar datos desde una hoja de cálculo.","info");
  const mockData = [
    {
