...
import { getTotalUnreadCount } from "./unread-messages-tracker.js";
...
const defaultProfileImageURL = "https://placehold.co/600x400?text=Sin+Imagen";
const auth = getAuth();
  ...
  if (user) {
    ...
    if (logguedInUserId) {
      ...
      if (docSnap.exists()) {
        ...
        // Buscar la imagen de perfil en múltiples ubicaciones posibles
        let profilePictureURL = null;
        if (userInfo?.["13_Foto de Perfil"]) {
          profilePictureURL = userInfo["13_Foto de Perfil"];
        ...
        if (userAvatar) {
          if (profilePictureURL) {
            ...
            userAvatar.onerror = () => {
              console.log(
                "Error cargando imagen de perfil, usando placeholder"
              );
...
function updateHeaderAvatar(imageUrl) {
  ...
  if (userAvatar && defaultIcon) {
    if (imageUrl) {
      ...
      userAvatar.onerror = () => {
        console.log("Error cargando imagen, mostrando icono por defecto");
...
const loadFeaturedProfessionals = async () => {
  ...
  if (featuredProfessionalsContainer) {
    ...
    try {
      ...
      featuredProfessionalsContainer.innerHTML = "";
      featuredProfessionalsContainer.style.display = "block";
      ...
      `;
        toggleProfessionalsButtons(false);
        return;
      }
      profCards = [];
      querySnapshot.forEach((doc) => {
        ...
        featuredProfessionalsContainer.style.display = "flex";
        ...
        `;
        professionalCard.addEventListener("click", (e) => {
          viewProfessionalProfile(doc.id);
        });
        profCards.push(professionalCard);
        featuredProfessionalsContainer.appendChild(professionalCard);
      });
      initProfessionalsCarousel();
    } catch (error) {
...
const loadFeaturedProjects = async () => {
  ...
  if (featuredProjectsContainer) {
    ...
    try {
      ...
      const querySnapshot = await getDocs(q);
      featuredProjectsContainer.innerHTML = "";
      featuredProjectsContainer.style.display = "block";
      if (querySnapshot.empty) {
        featuredProjectsContainer.innerHTML = `
       <div class="empty-results">
        <div class="empty-results-icon-x">
        ...
        `;
        toggleProjectsButtons(false);
        return;
      }
      projCards = [];
      querySnapshot.forEach((doc) => {
        const projectData = doc.data();
        let projectImage = "https://placehold.co/600x400?text=Sin+Imagen";
        if (projectData.images && projectData.images.length > 0) {
          if (projectData.images[0].secure_url) {
            projectImage = projectData.images[0].secure_url;
          }
        }
        featuredProjectsContainer.style.display = "flex";
...
function toggleProfessionalsButtons(show) {
  const prevBtn = document.getElementById("profPrev");
  const nextBtn = document.getElementById("profNext");
  if (prevBtn && nextBtn) {
...
function toggleProjectsButtons(show) {
...
function initProfessionalsCarousel() {
  const prevBtn = document.getElementById("profPrev");
  const nextBtn = document.getElementById("profNext");
  const container = document.getElementById("featuredProfessionals");
  const shouldShowButtons = profCards.length > 3;
  toggleProfessionalsButtons(shouldShowButtons);
  if (shouldShowButtons) {
    prevBtn.addEventListener("click", () => {
      if (profCurrentIndex > 0) {
        profCurrentIndex--;
        updateProfessionalsCarousel();
      }
    });
    nextBtn.addEventListener("click", () => {
      if (profCurrentIndex < profCards.length - 3) {
        profCurrentIndex++;
        updateProfessionalsCarousel();
      }
    });
...
function initProjectsCarousel() {
  const prevBtn = document.getElementById("projPrev");
  const nextBtn = document.getElementById("projNext");
  const shouldShowButtons = projCards.length > 3;
  toggleProjectsButtons(shouldShowButtons);
  if (shouldShowButtons) {
    prevBtn.addEventListener("click", () => {
      if (projCurrentIndex > 0) {
        projCurrentIndex--;
        updateProjectsCarousel();
      }
    });
    nextBtn.addEventListener("click", () => {
      if (projCurrentIndex < projCards.length - 3) {
        projCurrentIndex++;
        updateProjectsCarousel();
      }
    });
...
function updateProfessionalsCarousel() {
  const container = document.getElementById("featuredProfessionals");
  const containerWidth = container.parentElement.clientWidth - 30;
  const cardWidth = (containerWidth - 50) / 3;
  const translateX = profCurrentIndex * (cardWidth + 25);
...
function updateProjectsCarousel() {
  ...
  const containerWidth = container.parentElement.clientWidth - 30;
  const cardWidth = (containerWidth - 50) / 3;
  const translateX = projCurrentIndex * (cardWidth + 25);
  container.style.transform = `translateX(-${translateX}px)`;
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const mensajes = {
    ...
    Sugerencias:
      "Hola, querría compartir una sugerencia para mejorar la plataforma VivArq:",
  };
  ...
  if (emailForm) {
    emailForm.onsubmit = function (event) {
      ...
      if (!subject) {
        displayMessage(
          "Por favor selecciona un asunto antes de enviar.",
          "info"
        );
        return;
      }
