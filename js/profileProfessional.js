...
const DEFAULT_PROFILE_IMAGE = "https://placehold.co/400x400?text=Sin+Foto";
...
// En profileProfessional.js, reemplaza la función loadUserData con esta versión:
...
async function loadUserData(userId, isViewingOther = false) {
  try {
    ...
    if (userDoc.exists()) {
      ...
      // CORRECCIÓN: Buscar la imagen de perfil con prioridad correcta
      let profileImageUrl = null;

      // 1. Primero verificar si hay una imagen subida manualmente (tiene prioridad)
      if (userInfo?.profileImageUrl) {
        profileImageUrl = userInfo.profileImageUrl;
      }
      // 2. Si no hay imagen manual, usar la de Google (si existe)
      else if (userInfo?.["13_Foto de Perfil"]) {
        profileImageUrl = userInfo["13_Foto de Perfil"];
      }
      // 3. Ubicación alternativa
      else if (userData.profilePicture) {
        profileImageUrl = userData.profilePicture;
      }

      displayUserData(userInfo);
      updateProfileImageInUI(profileImageUrl || DEFAULT_PROFILE_IMAGE);
...
function adjustUIForOtherUserProfile(userId) {
  ...
  if (profileImageInput) profileImageInput.style.display = "none";
  if (uploadProfileImageBtn) uploadProfileImageBtn.style.display = "none";
...
async function saveProfileImageToFirestore(imageUrl) {
  try {
    ...
    // CORRECCIÓN: Cuando se sube una imagen manual, guardarla SOLO en profileImageUrl
    // Esto le da prioridad sobre la imagen de Google
    await updateDoc(userDocRef, {
      "userInfo.profileImageUrl": imageUrl,
      // No tocar el campo 13_Foto de Perfil para mantener la de Google como respaldo
    });

    updateProfileImageInUI(imageUrl);
    console.log("Imagen de perfil actualizada correctamente en Firestore");
  } catch (error) {
...
async function removeProfileImageFromFirestore() {
  try {
    ...
    if (userDoc.exists()) {
      ...
      const isGoogleAuth =
        userInfo?.["14_Método de Autenticación"] === "Google";
      const googlePhotoUrl = isGoogleAuth ? auth.currentUser?.photoURL : null;

      if (googlePhotoUrl) {
        // CORRECCIÓN: Si hay imagen de Google, NO restaurarla
        // En su lugar, eliminar ambos campos para que se muestre el placeholder
        await updateDoc(userDocRef, {
          "userInfo.profileImageUrl": deleteField(),
          "userInfo.13_Foto de Perfil": deleteField(),
        });
        updateProfileImageInUI(DEFAULT_PROFILE_IMAGE);
        displayMessage("Imagen de perfil eliminada con éxito", "success");
      } else {
        // No hay imagen de Google, eliminar normalmente
        await updateDoc(userDocRef, {
          "userInfo.profileImageUrl": deleteField(),
          "userInfo.13_Foto de Perfil": deleteField(),
        });
...
function updateProfileImageInUI(imageUrl) {
  ...
  if (profileImagePreview) {
    ...
    if (imageUrl && imageUrl !== DEFAULT_PROFILE_IMAGE) {
      // Hay una imagen real (de Google o subida)
      ...
      if (editProfileImageBtn) {
        editProfileImageBtn.style.display = "inline-block";
      }
    } else {
      ...
      placeholder.innerHTML =
        '<i class="fas fa-user fa-3x"></i><p>Sin foto de perfil</p>';
      profileImagePreview.appendChild(placeholder);
