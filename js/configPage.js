...
async function showDeleteModal() {
  ...
  const deletePasswordLabel = modal.querySelector(
  ...
  if (isGoogle) {
    // Usuario de Google: ocultar campo de contraseña y cambiar mensaje
    if (deletePasswordInput) {
      deletePasswordInput.parentElement.style.display = "none";
      deletePasswordInput.removeAttribute("required"); // Remover required para evitar error de validación
      deletePasswordInput.value = ""; // Limpiar valor
    }
    if (deletePasswordLabel) {
      deletePasswordLabel.style.display = "none";
    }
    if (modalDescription) {
      modalDescription.innerHTML = `
        <p>¿Estás seguro de que deseas eliminar tu cuenta?</p>
        <p style="color: #666; font-size: 14px; margin-top: 10px;">
          Iniciaste sesión con Google, por lo que no necesitas ingresar contraseña.
        </p>
        <p style="color: #d32f2f; font-weight: 500; margin-top: 10px;">
    ...
    // Usuario con contraseña normal: mostrar campo y mensaje normal
    if (deletePasswordInput) {
      deletePasswordInput.parentElement.style.display = "block";
      deletePasswordInput.setAttribute("required", ""); // Asegurar que required esté presente
      deletePasswordInput.value = "";
    }
    if (deletePasswordLabel) {
      deletePasswordLabel.style.display = "block";
    }
    if (modalDescription) {
      modalDescription.innerHTML = `
        <p>¿Estás seguro de que deseas eliminar tu cuenta?</p>
...
function hideDeleteModal() {
  ...
  if (deletePasswordInput) {
    deletePasswordInput.value = "";
    deletePasswordInput.parentElement.style.display = "block";
    deletePasswordInput.setAttribute("required", ""); // Restaurar required al cerrar
  }
  const deletePasswordLabel = modal.querySelector(
    'label[for="delete-password"]'
  );
...
async function handleAccountDeletion(e) {
  ...
  try {
    ...
    if (isGoogle) {
      // Para usuarios de Google, reautenticar con popup de Google
      const provider = new GoogleAuthProvider();
      await reauthenticateWithPopup(user, provider);
    } else {
      // Para usuarios con contraseña, reautenticar con contraseña
    ...
    if (error.code === "auth/wrong-password") {
      ...
      displayMessage(
        "Por favor, cierra sesión y vuelve a iniciar para eliminar tu cuenta",
        "error"
      );
    } else {
      displayMessage(
