...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function showValidationError() {
    displayMessage("Por favor complete todos los campos obligatorios antes de continuar.","warning");
  }
  function saveDraft() {
  ...
  async function publishProject(e) {
    ...
    try {
      ...
      displayMessage("¡Proyecto publicado exitosamente!","success");
      window.location.href = `profile-contractor.html?id=${userId}`;
    } catch (error) {
      console.error("Error al publicar proyecto:", error);
      displayMessage("Error al publicar el proyecto. Por favor intente nuevamente.","error");
  ...
  function handleFileUpload() {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (!["image/jpeg", "image/png", "application/pdf"].includes(file.type)) {
        displayMessage(`El archivo ${file.name} no es un formato válido (JPG, PNG o PDF).`,"warning");
        continue;
      }
  ...
  function addReferenceUrl() {
    ...
    if (!url) {
      displayMessage("Por favor ingrese una URL válida.","warning");
      return;
    }
    try {
      new URL(url);
