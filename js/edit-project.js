...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!projectId) {
    displayMessage("ID de proyecto no proporcionado", "error");
  ...
  async function loadProjectData() {
    try {
      ...
      if (!projectSnap.exists()) {
        displayMessage("El proyecto no existe", "error");
        window.location.href = "index.html";
        return;
      }
      const projectData = projectSnap.data();
      if (projectData.userId !== userId) {
        displayMessage("No tienes permiso para editar este proyecto", "error");
        window.location.href = "index.html";
        return;
      }
      ...
      displayMessage("Error al cargar el proyecto. Por favor intente nuevamente.", "error");
    }
  }
  ...
  function showValidationError() {
    displayMessage("Por favor complete todos los campos obligatorios antes de continuar.", "info");
  }
  function saveDraft() {
    ...
    updateInFirestore(formData)
      .then(() => {
        displayMessage("Proyecto guardado como borrador. Puede continuar editándolo más tarde.", "success");
        window.location.href = `profile-contractor.html?id=${userId}`;
      })
      .catch((error) => {
        console.error("Error al guardar borrador:", error);
        displayMessage("Error al guardar el borrador. Por favor intente nuevamente.", "error");
      });
  }
  function updateProject(e) {
    ...
    Promise.all(processImagePromises)
      ...
      .then(() => {
        displayMessage("¡Proyecto actualizado exitosamente!", "success");
        window.location.href = `profile-contractor.html?id=${userId}`;
      })
      .catch((error) => {
        ...
        displayMessage("Error al actualizar el proyecto. Por favor intente nuevamente.", "error");
      });
  }
  ...
  function handleFileUpload() {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (file.size > 2 * 1024 * 1024) {
        displayMessage(`El archivo ${file.name} excede el tamaño máximo recomendado de 2MB. Las imágenes grandes pueden causar problemas al guardar.`, "warning");
      }
      if (!["image/jpeg", "image/png", "application/pdf"].includes(file.type)) {
        displayMessage(`El archivo ${file.name} no es un formato válido (JPG, PNG o PDF).`, "error");
        continue;
      }
  ...
  function addReferenceUrl() {
    ...
    if (!url) {
      displayMessage("Por favor ingrese una URL válida.", "error");
      return;
    }
    try {
      new URL(url);
    } catch (e) {
      displayMessage("Por favor ingrese una URL válida que incluya http:// o https://", "warning");
      return;
    }
